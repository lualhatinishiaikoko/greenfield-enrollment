...
<style>
  /* ─── Login card: flat revised look (login_revised.php) ──────────────────
     Scoped to this page only — teacher_login.php and student_login.php
     still use css_staff.css's glassmorphism .modal, so these overrides
     live here instead of in the shared stylesheet. Popup open/close JS,
     the landing screen, and the form/PHP logic below are all unchanged;
     only the card's visuals are swapped out. */
  :root {
    --login-ink: #1B1F1D;
    --login-muted: #6B7570;
    --login-border: #DCE1DD;
