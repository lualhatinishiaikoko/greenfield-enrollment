...
// (lms_home.php, my_courses.php, student_lessons.php, etc.).
...
<script src="<?= $base ?>js/tab_guard.js?v=<?= filemtime(__DIR__ . '/../assets/js/tab_guard.js') ?>" data-token="<?= htmlspecialchars($_SESSION['sg_tab_token'] ?? '', ENT_QUOTES) ?>" data-storage-key="sg_tab_token_lms" data-logout-url="<?= $base ?>learningportal/lms_logout"></script>
...
<nav class="lms-foldernav" id="lmsFolderNav">
  <div class="lms-foldernav-inner">
    <a href="<?= $base ?>learningportal/lms_home" class="lms-foldernav-logo">
      <img src="<?= $base ?>images/log_ui.png" alt="Greenfield Senior High School">
    </a>
    ...
    <div class="lms-foldernav-tabs" id="lmsFolderNavTabs">
      <?php lmsNavTab($base . 'learningportal/lms_home', 'Overview', $current, 'lms_home.php'); ?>
      <?php lmsNavTab($base . 'learningportal/my_courses', 'My Courses', $current, 'my_courses.php'); ?>
      ...
      <div class="lms-navtab-dropdown<?= $academicsActive ? ' open-active' : '' ?>" id="lmsAcademicsDropdown">
        ...
        <div class="lms-navtab-panel">
          <?php lmsNavPanelLink($base . 'learningportal/student_lessons', 'Lessons', $current, 'student_lessons.php'); ?>
          <?php lmsNavPanelLink($base . 'learningportal/student_discussions', 'Discussions', $current, 'student_discussions.php'); ?>
          <?php lmsNavPanelLink($base . 'learningportal/student_assignments', 'Assignments', $current, 'student_assignments.php'); ?>
          <?php lmsNavPanelLink($base . 'learningportal/student_quizzes', 'Quizzes', $current, 'student_quizzes.php'); ?>
        </div>
      </div>
    </div>
    ...
    <div class="lms-foldernav-right">
        ...
        <div class="lms-notif-dropdown" id="lmsNotifDropdown">
          ...
          <?php else: ?>
            <?php foreach ($lms_notifs as $lms_n): ?>
              <a href="<?= $base . htmlspecialchars($lms_n['link']) ?>" class="lms-notif-item<?= $lms_n['is_read'] ? '' : ' unread' ?>">
      ...
      <div class="lms-foldernav-profile-wrapper" id="lmsProfileWrapper">
        <button type="button" class="lms-foldernav-profile" id="lmsProfileToggle" aria-haspopup="true" aria-expanded="false" aria-label="<?= htmlspecialchars($profile_name) ?> — account menu" title="<?= htmlspecialchars($profile_name) ?>">
          <span class="lms-foldernav-avatar">
            <?php if (!empty($sb_photo_path)): ?>
              <img src="<?= $base ?>studentportal/student_photo?portal=lms" alt="">
            <?php else: ?>
        ...
        <div class="lms-foldernav-dropdown" id="lmsProfileDropdown">
          <a href="<?= $base ?>studentportal/student_profile?portal=lms" class="lms-foldernav-dropdown-item"><span>Profile</span></a>
          <a href="<?= $base ?>learningportal/lms_logout" class="lms-foldernav-dropdown-item lms-foldernav-dropdown-logout" id="lmsFolderLogoutBtn"><span>Logout</span></a>
        </div>
      </div>
    </div>
  </div>
    ...
    if (notifBell && notifDropdown) {
      ...
      notifBell.addEventListener('click', function (e) {
        ...
        if (notifDropdown.classList.contains('open') && !notifMarked) {
          notifMarked = true;
          fetch('<?= $base ?>ajax/lms_mark_notifications_read', { method: 'POST' });
      ...
      logoutBtn.addEventListener('click', function (e) {
        ...
        if (typeof Swal === 'undefined') {
          if (window.confirm('Log out? You will be returned to the login page.')) {
            ...
            window.location.href = '<?= $base ?>learningportal/lms_logout';
          }
          return;
        }
        Swal.fire({
