...
//
// Included from the other roles/student/portal/student_*.php pages (same
// folder). Every link below uses the absolute APP_URL prefix instead of a
...
<script src="<?= APP_URL ?>/assets/js/tab_guard.js?v=<?= filemtime(__DIR__ . '/../../assets/js/tab_guard.js') ?>" data-token="<?= htmlspecialchars($_SESSION['sg_tab_token'] ?? '', ENT_QUOTES) ?>" data-storage-key="sg_tab_token_student" data-logout-url="<?= APP_URL ?>/roles/student/portal/student_logout"></script>
...
<aside class="student-sidebar" id="studentSidebar">
  ...
  <div class="student-sidebar-inner">
    ...
    <a class="student-sidebar-brand" href="<?= APP_URL ?>/roles/student/portal/student_dashboard" title="Go to Dashboard">
      <span class="student-sidebar-logo-wrap">
        <img class="student-sidebar-logo" src="<?= APP_URL ?>/assets/images/logo_sidebar_v3.png" alt="Greenfield Senior High School">
      </span>
    </a>

    <nav class="student-sidebar-nav">
      <span class="student-nav-label">Overview</span>
      <?php studentNavLink(APP_URL . '/roles/student/portal/student_dashboard', 'Dashboard', $ico_dashboard, $current, 'student_dashboard.php'); ?>

      <span class="student-nav-label">Academics</span>
      <?php studentNavLink(APP_URL . '/roles/student/portal/student_schedule', 'My Schedule', $ico_schedule, $current, 'student_schedule.php'); ?>
      <?php studentNavLink(APP_URL . '/roles/student/portal/student_grades', 'My Grades', $ico_grades, $current, 'student_grades.php'); ?>

      <span class="student-nav-label">Finance</span>
      <?php studentNavLink(APP_URL . '/roles/student/portal/student_payments', 'My Payments', $ico_payment, $current, 'student_payments.php'); ?>
      <?php studentNavLink(APP_URL . '/roles/student/portal/student_pay_online', 'Pay Online', $ico_payment, $current, 'student_pay_online.php'); ?>
      <?php studentNavLink(APP_URL . '/roles/student/portal/student_accountabilities', 'Accountabilities', $ico_accountability, $current, 'student_accountabilities.php'); ?>
    </nav>
    ...
    <div class="student-sidebar-footer">
      <div class="student-sidebar-user">
        <div class="student-sidebar-avatar">
          <?php if (!empty($sb_photo_path)): ?>
            <img src="<?= APP_URL ?>/roles/student/portal/student_photo" alt="" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">
          <?php else: ?>
      ...
      <a href="<?= APP_URL ?>/roles/student/portal/student_logout" class="student-btn-logout" id="studentLogoutBtn" title="Log out">
      ...
      logoutBtn.addEventListener('click', function (e) {
        ...
        if (typeof Swal === 'undefined') {
          if (window.confirm('Log out? You will be returned to the login page.')) {
            ...
            window.location.href = '<?= APP_URL ?>/roles/student/portal/student_logout';
          }
          return;
        }
        Swal.fire({
