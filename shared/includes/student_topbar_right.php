...
// (for $sb_given_name / $sb_family_name / $sb_photo_path). Self-contained
...
// every roles/student/portal/*.php page, so the topbar look stays identical everywhere
...
$tbr_profile_href = APP_URL . '/roles/student/portal/student_profile' . ($tbr_is_lms ? '?portal=lms' : '');
$tbr_logout_href  = APP_URL . ($tbr_is_lms ? '/roles/lms/lms_logout' : '/roles/student/portal/student_logout');
$tbr_photo_href   = APP_URL . '/roles/student/portal/student_photo' . ($tbr_is_lms ? '?portal=lms' : '');
...
$tbr_mark_read_url = APP_URL . '/ajax/' . ($tbr_is_lms ? 'lms_mark_notifications_read' : 'student_mark_notifications_read');
    ...
    <div class="student-notif-dropdown" id="studentNotifDropdown">
      ...
      <?php else: ?>
        <?php foreach ($tbr_notifs as $tbr_n): ?>
          <a href="<?= APP_URL . '/' . htmlspecialchars($tbr_n['link']) ?>" class="student-notif-item<?= $tbr_n['is_read'] ? '' : ' unread' ?>">
